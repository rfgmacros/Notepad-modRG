...
- (void) tile {
	...
	if (std::floor(NSAppKitVersionNumber) > NSAppKitVersionNumber10_13) {
		const CGFloat rulerWidth = self.verticalRulerView.requiredThickness;
		NSRect contentFrame = self.contentView.frame;
		if (_rtlMode) {
			// RTL: ruler (line numbers/bookmarks) on the right side.
			// Word wrap is forced on in RTL so no horizontal scrollbar.
			contentFrame.origin.x = 0;
			contentFrame.size.width = NSWidth(self.frame) - rulerWidth;
			self.contentView.frame = contentFrame;
			NSRect rulerFrame = self.verticalRulerView.frame;
			rulerFrame.origin.x = NSWidth(self.frame) - rulerWidth;
			rulerFrame.size.width = rulerWidth;
			self.verticalRulerView.frame = rulerFrame;
		} else {
			// LTR: ruler on the left (default)
			contentFrame.origin.x = rulerWidth;
...
- (void) mouseDown: (NSEvent *) theEvent {
	...
	// RTL mode needs MarginMouseDown for correct X coordinate remapping.
	// LTR mode uses the original MouseDown path — proven and unchanged.
	if (((SCIScrollView *)self.scrollView).rtlMode)
		owner.backend->MarginMouseDown(theEvent, self);
	else
...
/**
 * Use popUpMenuPositioningItem instead of popUpContextMenu to prevent macOS
 * from injecting system items (AutoFill, Services, Writing Tools) into the menu.
 */
- (void) rightMouseDown: (NSEvent *) theEvent {
	NSMenu *menu = [self menuForEvent: theEvent];
	if (menu) {
