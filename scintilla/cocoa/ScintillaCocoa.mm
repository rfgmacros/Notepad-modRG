...
void ScintillaCocoa::MarginMouseDown(NSEvent *event, NSView *marginView) {
	// Use ConvertPoint (same as the original MouseDown) to get correct
	// document-space coordinates, then override X for margin detection.
	// This ensures Y coordinate handling is identical to content-view clicks.
	Point location = ConvertPoint(event.locationInWindow);
	// Override X: convert margin-local X to the expected [-fixedColumnWidth, 0] range
	NSPoint local = [marginView convertPoint:event.locationInWindow fromView:nil];
	location.x = local.x - vs.fixedColumnWidth;
	ButtonDownWithModifiers(location, TimeOfEvent(event),
				TranslateModifierFlags(event.modifierFlags));
}
